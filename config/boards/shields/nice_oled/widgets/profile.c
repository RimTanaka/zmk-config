...
static void draw_inactive_profiles(lv_obj_t *canvas,
  ...
  lv_canvas_draw_img(canvas, 0, 71, &profiles, &img_dsc);
...
static void draw_active_profile(lv_obj_t *canvas,
  ...
  lv_canvas_draw_rect(canvas, -1 + offset, 70, 5, 5, &rect_white_dsc);
...
static void draw_active_profile_text(lv_obj_t *canvas,
  ...
  init_label_dsc(&label_dsc, LVGL_FOREGROUND, &pixel_operator_mono_10,
                 LV_TEXT_ALIGN_LEFT);
  ...
  lv_canvas_draw_text(canvas, 25, 50, 35, &label_dsc, text);
}
